...
class Articles extends Application
{
    ...
    public function index(Request $request)
    {
        ...
        $list = ArticlesModel::where($condition)->paginate(20, false, ['query' => $request->param()]);
        $children = [];
    ...
    {
        $data = $request->post();
        $validate = $this->validate($data, 'Articles');
        if(true !== $validate){
        ...
        if($result){
            $this->success('新增成功', '/backend/articles?tid='.$request->post('tid'));
        } else {
            $this->error('新增失败');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($result){
            $this->success('修改成功', '/backend/articles?tid='.$request->post('tid'));
        } else {
            $this->error('修改失败');
        }
    }
    ...
    public function change_taxon(Request $request) {
      ...
      $ids = $request->post('ids');
      $ids_array = explode(',', $ids);
      $result = ArticlesModel::whereIn('id', $ids_array)->update(['tid' => $tid]);
      if($result){
